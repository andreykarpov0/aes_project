...
string readFile(string path)
{
  ...
  // vector<unsigned char> buffer(std::istreambuf_iterator<unsigned char>(fInput), {});
	string buffer(std::istreambuf_iterator<unsigned char>(fin), {});
	fin.close();
	return buffer;
}
...
string encrypt(string text, string key)
{
	...
	vector <vector< vector< unsigned char > > > roundkeys(1,vector<vector<unsigned char > >(4, vector<unsigned char>(4, 0)));
	if(text.size() % 16)
	{
