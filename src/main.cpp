...
int main() 
{
	string pathin, pathout, text, password, res, a;
	int x = 0;
	...
	vector<vector <unsigned char > > key = keyexpansion(password, text.size());
	cout << x;
	if(x == 1)
	{
		cout << 1;
...
string encrypt(string text, vector<vector <unsigned char > > key)
{
	...
	for(int i = 0; i < blockText.size(); ++i)
	{
		for(int j = 0; j < blockText[i].size(); ++j)
		{
			for(int k = 0; k < blockText[i][j].size(); ++k)
			{
                ...
                t = text[count];
                blockText[i][j][k] = (unsigned char)sbox[t];
                ++count;
			}
		}
	}
	...
	for(int i = 0; i < blockText.size(); ++i)
	{
		for(int j = 0; j < blockText[i].size(); ++j)
		{
			for(int k = 0; k < blockText[i][j].size(); ++k)
			{
				result += blockText[i][j][k]^key[j][k + i*4];
			}
		}
	}
	return result;
...
string decrypt(string text, vector<vector <unsigned char > > key)
{
	...
	for(int i = 0; i < blockText.size(); ++i)
	{
		for(int j = 0; j < blockText[i].size(); ++j)
		{
			for(int k = 0; k < blockText[i][j].size(); ++k)
			{
				blockText[i][j][k] = text[count]^key[j][k+(i*4)];
			}
		}
	}
	count = 0;
		...
		for(int j = 0; j < blockText[i].size(); ++j)
		{
			for(int k = 0; k < blockText[i][j].size(); ++k)
			{
				if(count >= text.size() * 16)
					break;
                t = blockText[i][j][k];
				blockText[i][j][k] = inv_sbox[(int)t];
				result += blockText[i][j][k];
				++count;
			}
		}
	}
